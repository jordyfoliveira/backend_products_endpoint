...
async def fake_current_user():
    return {"username": "jordy", "role": "ADMIN"}

async def fake_list_products(limit=10, offset=0, is_active=None, name=None, sku=None):
    return [{"id": 20260001, "display_id": "2026-0001", "sku": "SKU001", "name": "Produto Teste", "description": "Teste", "price": 10.5, "stock": 5, "is_active": True, "created_at": "2026-01-01T00:00:00", "updated_at": "2026-01-01T00:00:00",}]
...
async def fake_create_product(product, username):
    return {"id": 20260002, "display_id": "2026-0002", "created_by": username, "message": "Produto criado com sucesso!"}

def test_create_product(monkeypatch):
    monkeypatch.setattr(product_service, "create_product", fake_create_product)
    payload = {"sku": "SKU002", "name": "Produto Teste", "description": "Produto criado em teste", "price": 20.5, "stock": 10}
    ...
    assert response.json()["created_by"] == "jordy"

async def fake_get_product_by_id_none(product_id):
    return None

async def fake_get_product_by_id_success(product_id):
    return {"id": 20260001, "display_id": "2026-0001", "sku": "SKU001", "name": "Mouse", "price": 19.99, "stock": 10}

def test_get_product_not_found(monkeypatch):
    monkeypatch.setattr(product_service, "get_product_by_id", fake_get_product_by_id_none)
    app.dependency_overrides[require_manager_or_admin] = fake_current_user
...
def test_get_product_by_id(monkeypatch):
    monkeypatch.setattr(product_service, "get_product_by_id", fake_get_product_by_id_success)
    app.dependency_overrides[require_manager_or_admin] = fake_current_user
